...
export const guides: Guide[] = [
 g("introduction","Getting started","Introduction","Build collections, payouts and treasury workflows in Kenyan Shillings.",[
  s("what-is-rahapay","One KES integration",["Rahapay gives Kenyan businesses one API for accepting customer payments, holding funds, paying recipients and moving value into or out of KES. Every request is scoped to your merchant account and every money movement has a unique transaction ID."],{callout:{kind:"note",title:"KES only",text:"This documentation covers Kenyan Shilling products. All amount examples are denominated in KES unless a conversion explicitly names the other side of the pair."}}),
  s("core-concepts","Core concepts",["A merchant is the business integrating Rahapay. A customer pays the merchant through a collection. A payout moves available funds to a beneficiary. Balances separate available funds from amounts still pending. Conversions exchange funds when one side of the pair is KES."],{table:{headers:["Concept","Meaning"],rows:[["Collection","Money received from a customer"],["Payout","Money sent to a bank account or mobile wallet"],["Available balance","Funds ready to pay out or convert"],["Pending balance","Funds awaiting final confirmation"]]}}),
  s("first-flow","Your first flow",["Start in Sandbox, fund your test KES balance, create a collection, and listen for its webhook. Move to Live only after signature verification, idempotency and reconciliation are working end to end."],{bullets:["Use a unique merchant reference for every write.","Treat API responses as acknowledgements; use webhooks or retrieval endpoints for final status.","Store Rahapay IDs alongside your own references."]})
 ]),
 ...
 g("authentication","Getting started","Authentication","Authenticate each request with an environment-specific Rahapay API key.",[
 ...
 g("rate-limits","Getting started","Rate Limits","Design clients that remain reliable when request volume rises.",[
  s("limits","How limits work",["Limits apply per merchant, environment and route family. Responses include limit, remaining and reset headers. When the limit is exceeded, Rahapay returns HTTP 429 with a retry delay."],{table:{headers:["Header","Meaning"],rows:[["X-RateLimit-Limit","Requests allowed in the current window"],["X-RateLimit-Remaining","Requests still available"],["X-RateLimit-Reset","Unix time when the window resets"],["Retry-After","Seconds before another attempt"]]}}),
 ...
 g("webhooks-setup","Webhooks","Setting Up Webhooks","Receive signed HTTPS notifications when transaction states change.",[
  ...
  s("delivery","Delivery lifecycle",["Rahapay signs the exact raw request body, sends it to your endpoint, and records your response. A non-2xx response or timeout schedules another attempt."],{diagram:"webhook"})
 ]),
 g("verify-webhooks","Webhooks","Verifying Webhook Signatures","Authenticate webhook deliveries before trusting their contents.",[
  ...
  s("node","Node.js",["Use the built-in crypto module."],{code:"const expected = createHmac('sha512', process.env.RAHAPAY_WEBHOOK_SECRET)\n  .update(rawBody)\n  .digest('hex');\nconst valid = timingSafeEqual(Buffer.from(expected), Buffer.from(signature));"}),
 ...
 g("collection-webhook","Webhooks","Collection Webhook","Handle collection progress and final payment outcomes.",[
  ...
  s("payload","Important fields",["Use `data.id` as the Rahapay identifier, `data.reference` to join your order, `data.amountReceived` for reconciliation, and `data.status` for state. Never mark an order paid from the event name alone without checking the amount and currency."],{code:'{\n  "id": "evt_01J8COL",\n  "type": "collection.completed",\n  "data": { "id": "col_01J8KFAKE", "reference": "ord_ke_10492", "amount": 2500, "amountReceived": 2500, "currency": "KES", "status": "successful" }\n}'})
 ]),
 ...
 g("checkout","Collections (pay-in)","Checkout","Offer a Rahapay-hosted redirect or inline KES checkout.",[
  s("hosted","Hosted redirect",["Create a checkout on your server, then redirect the customer to the returned URL. After payment, Rahapay returns the browser to your redirect URL. Verify the collection server-side before fulfilling the order."]),
 ...
 g("mobile-money","Collections (pay-in)","M-Pesa and Mobile Money","Collect KES through an STK prompt sent to a Kenyan mobile wallet.",[
  s("flow","STK push flow",["Your server creates a charge with the KES amount, Kenyan phone number, network and reference. Rahapay asks the network to prompt the customer. The customer confirms with their wallet PIN, and Rahapay sends the final event."],{diagram:"mobile"}),
 ...
 g("bank-transfer","Collections (pay-in)","Pay with Bank Transfer","Create transfer instructions and reconcile the incoming KES credit.",[
  ...
  s("matching","Matching payments",["Rahapay matches the bank credit to the generated instruction. Store the collection ID and merchant reference. Compare `amountReceived` with the expected amount before fulfilling."])
 ]),
 ...
 g("payouts-overview","Payouts","Overview","Send KES to Kenyan bank accounts and mobile wallets.",[
  ...
  s("lifecycle","Payout lifecycle",["Rahapay validates the request, reserves balance, submits it to the destination network, and sends the final result."],{diagram:"payout"}),
 ...
 g("bank-payouts","Payouts","Bank Transfers to Kenyan Banks","Pay a verified account at a supported Kenyan bank.",[
  ...
  s("processing","Process the result",["A successful create response usually means processing has begun. Listen for `payout.completed`, `payout.failed` or `payout.reversed` and provide support with the Rahapay payout ID."])
 ]),
 ...
 g("cross-currency-payouts","Payouts","Cross-currency Payouts into KES","Quote the conversion, execute it, then pay the recipient from your KES balance.",[
  ...
  s("convert","2. Execute before expiry",["Execute the quote once. On success, Rahapay debits the source balance and credits KES. If the quote expires, request a new one rather than reusing its rate."]),
 ...
 g("error-codes","Reference","Error Codes","Handle Rahapay failures with stable machine-readable codes.",[
 ...
 g("institutions","Reference","Supported Banks and Mobile Networks","Discover active KES payout destinations at runtime.",[
  ...
  s("networks","Mobile networks",["Rahapay supports KES wallet flows for M-Pesa and Airtel Money where enabled for your merchant. Pass `MPESA` or `AIRTEL_MONEY` exactly as shown."])
 ]),
 g("changelog","Reference","Changelog","Track changes to Rahapay’s KES API and documentation.",[
 ...
 g("faq","Reference","FAQ","Answers to common KES integration questions.",[
  ...
  s("support","How do I get help?",["Email `{{SUPPORT_EMAIL}}` with the request ID, Rahapay resource ID, environment and timestamp. Never include a full secret key or wallet PIN."])
 ])
];
