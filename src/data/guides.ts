...
export const guides:Guide[]=[
 ...
 g("hosted-checkout","Accept payments","Hosted checkout","Create a Raha Pay page where your customer chooses how to pay.",[
  s("purpose","Send the customer to checkout",["Your server creates the checkout and redirects the customer to the returned URL."],{callout:{kind:"warning",title:"Verify before fulfilment",text:"A customer can close or revisit the browser page. Confirm the payment on your server."}}),request("Send `amount`, `reference`, `customer` and `callback_url` to `/v1/payments`."),response("Open `data.checkout_url` in the customer’s browser."),webhook("Treat the browser return as navigation only. Use the signed payment event as the result."),errors([["invalid_callback_url","400","Send a public HTTPS URL."],["duplicate_reference","409","Use a new reference."]])
 ]),
