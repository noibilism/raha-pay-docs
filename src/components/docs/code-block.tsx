...
export function CodeBlock({ code, label = "Example" }: { code: string; label?: string }) {
  ...
  return <div className="code-shell my-5 overflow-hidden rounded-[10px] border border-code-border bg-code text-code-foreground">
    <div className="flex h-10 items-center justify-between border-b border-code-border px-4 text-xs text-code-muted"><span>{label}</span><div className="flex items-center gap-1"><Button variant="ghost" size="sm" className="h-7 px-2 text-code-muted hover:bg-code-hover hover:text-code-foreground" onClick={()=>openAssistant(window.location.pathname, rendered)}><Bot/>Explain</Button><Button variant="ghost" size="icon" className="h-7 w-7 text-code-muted hover:bg-code-hover hover:text-code-foreground" onClick={copy} aria-label="Copy code">{copied ? <Check /> : <Copy />}</Button></div></div>
